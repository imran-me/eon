...
export const RULES = [
  { id: 'vat-return', kind: 'monthly', day: 15, name: 'VAT return — Mushak 9.1 for the previous month', basis: 'VAT & SD Act 2012 s.64 and VAT & SD Rules 2016: the return for each tax period is filed by the 15th of the following month, with the net VAT payable deposited by the same date.', match: /vat|mushak/i },
  { id: 'vds-deposit', kind: 'monthly', day: 7, name: 'VAT deducted at source (VDS) — treasury deposit', basis: 'VAT & SD Act 2012 s.49 with the VDS rules: VAT withheld from suppliers is deposited to the treasury and Mushak 6.6 issued to the supplier soon after deduction (within the first week of the following month in current practice).', note: VERIFY, match: /\bvds\b|vat (deposit|withh)|withholding vat/i },
  { id: 'tds-salary', kind: 'monthly', day: 14, name: 'TDS on salaries — monthly treasury deposit', basis: 'Income Tax Act 2023 s.86 (tax deducted from salary at the average rate) read with the withholding rules: tax deducted in a month is deposited within two weeks after the month ends (June deductions on a shorter timeline).', note: VERIFY, match: /\btds\b|salary tax|withholding tax deposit|source tax/i },
  { id: 'wages', kind: 'monthly', day: 7, name: 'Wages for the previous month paid to all workers', basis: 'Bangladesh Labour Act 2006 s.123: wages are payable within seven working days after the end of the wage period. EON uses the 7th of the month as the practical line.', match: /payroll|salary|salaries|wages/i, erp: 'payroll' },
  { id: 'wht-return', kind: 'dates', dates: [[1, 31], [7, 31]], name: 'Half-yearly withholding tax return', basis: 'Income Tax Act 2023 s.177 (successor of s.75A of the 1984 Ordinance): every tax-deducting authority files a withholding return twice a year — by 31 January and 31 July.', note: VERIFY, match: /withholding (tax )?return|75a|\b177\b/i },
  { id: 'salary-statement', kind: 'annual', month: 9, day: 1, name: 'Annual statement of salaries paid and tax deducted', basis: 'Income Tax Rules: the annual salary and TDS statement for the income year (with employee TINs) is filed by 1 September.', note: VERIFY, match: /salary statement|annual (tds|salary) return|tds return/i },
  { id: 'ait', kind: 'dates', dates: [[9, 15], [12, 15], [3, 15], [6, 15]], name: 'Advance income tax — quarterly instalment', basis: 'Income Tax Act 2023 s.154–155: where the last assessed income exceeds the threshold, advance tax is paid in four equal instalments on 15 September, 15 December, 15 March and 15 June.', match: /advance (income )?tax|quarterly tax|\bait\b/i },
  { id: 'company-return', kind: 'annual', month: 1, day: 15, name: 'Annual income tax return of each company — Tax Day', basis: 'Income Tax Act 2023 s.2(23) “Tax Day” for companies: the 15th day of the seventh month after the end of the income year — 15 January for a June year-end. Audited accounts under the Companies Act must accompany the return.', note: 'if a company has a different income year, its Tax Day moves accordingly', match: /income tax return|tax return|corporate tax|return of income/i },
  { id: 'tin-bin', kind: 'annual', month: 1, day: 31, name: 'TIN & BIN status and proof-of-return check for every company', basis: 'Income Tax Act 2023 s.264 (proof of submission of return is required for licences, bank facilities and tenders) and VAT & SD Act 2012 s.4–6 (BIN registration must reflect current address, activity and turnover). TIN and BIN do not lapse, but the yearly proof of return does — refresh it right after Tax Day.', match: /\btin\b|\bbin\b|proof of (return|submission)|\bpsr\b/i },
  { id: 'trade-licence', kind: 'annual', month: 6, day: 30, name: 'Trade licence renewal — every company and branch office', basis: 'Local Government (City Corporation) Act 2009 with the corporation’s Model Tax Schedule: trade licences run per fiscal year and are renewed by 30 June; late renewal attracts a surcharge.', match: /trade licen[cs]e/i },
  { id: 'fire-licence', kind: 'annual', month: 6, day: 30, name: 'Fire licence renewal', basis: 'Fire Prevention and Extinction Act 2003 s.4–5: premises need a licence from the Fire Service & Civil Defence, renewed annually with the fire-safety plan and drill record.', note: 'the due date follows your licence expiry — ' + VERIFY, match: /fire (licen[cs]e|safety|certificate)/i },
  { id: 'agm', kind: 'annual', month: 12, day: 31, name: 'Annual General Meeting of each company', basis: 'Companies Act 1994 s.81: an AGM in every calendar year, not more than 15 months after the previous one (first AGM within 18 months of incorporation). Audited accounts and auditor appointment go before it.', note: 'enter your last AGM date with the company secretary — the 15-month clock runs from it', match: /\bagm\b|annual general meeting|board meeting/i },
  { id: 'rjsc-return', kind: 'annual', month: 1, day: 21, name: 'Annual return to RJSC (Schedule X) with audited accounts', basis: 'Companies Act 1994 s.36 and s.190: the annual return with the list of members is filed with the Registrar of Joint Stock Companies within 21 days of the AGM; the balance sheet within 30 days of the AGM.', note: 'dated 21 days after a 31 December AGM — moves with your AGM', match: /\brjsc\b|annual return|schedule x|registrar/i },
];
...
export function calendar(today, D, company = null) {
  ...
  RULES.forEach((rule) => {
    occurrences(rule, today).forEach((o) => {
      ...
      let it = { id: rule.id, date: o.date, days: daysBetween(today, o.date), name: rule.name, basis: rule.basis, note: rule.note || null, period: periodLabel(rule, o), source: 'statute', todo: todo ? { id: todo.id, title: todo.title, status: todo.status, due_date: todo.due_date, company: coName(todo.company_id) } : null, amount: 0 };
