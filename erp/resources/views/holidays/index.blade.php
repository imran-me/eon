...
@section('main-content')
        ...
        <div class="states-table-container">
            <div class="states-table-header bg-blue-600 px-6 py-4 flex justify-between items-center">
                <h2 class="states-table-title text-white text-xl font-semibold" style="color: white">
                    <i class="fas fa-list mr-2"></i>Holiday List
                </h2>
                @can('create holiday')
                        ...
                        <tbody class="bg-white divide-y divide-gray-200">
                            @forelse ($datas as $key => $value)                       
                                <tr>
                                    ...
                                    <td class="px-6 py-4 whitespace-nowrap">{{ $value->start_date }}</td>                                                                     
                                    <td class="px-6 py-4 whitespace-nowrap">{{ $value->end_date }}</td>                                                                     
                                    ...
                                    <td class="px-6 py-4 whitespace-nowrap">
                                        <div class="btn-group btn-group-sm flex space-x-1">
                                            ...
                                            <button class="btn btn-outline-primary edit-btn border border-blue-500 text-blue-500 hover:bg-blue-500 hover:text-white px-3 py-1 rounded-md transition duration-200" 
                                                ...
                                                data-item_start_date="{{ $value->start_date }}" 
                                                data-item_end_date="{{ $value->end_date }}" 
                                                data-item_note="{{ $value->note }}" 
                                                data-action="{{ route('role.holidays.update', ['role' => Str::slug(Auth::user()->getRoleNames()->first()), 'holiday' => $value->id]) }}" title="Edit Item">
    ...
    @include('holidays.create-modal')
    @include('holidays.edit-modal')
    @include('holidays.delete-modal')
    ...
    <script>
        $(document).ready(function() {
            ...
            $('.edit-btn').click(function() {
                ...
                const item_start_date = $(this).data('item_start_date');
                const item_end_date = $(this).data('item_end_date');
                const item_note = $(this).data('item_note');
                
                // Set values in the edit form
                $('#editItemId').val(item_id);
                $('#edit_name').val(item_name);
                $('#edit_start_date').val(item_start_date);
                $('#edit_end_date').val(item_end_date);
                $('#edit_note').val(item_note);
                    ...
                    $.ajax({
                        ...
                        error: function (xhr) {
                            ...
                            Swal.fire({
                                icon: 'error',
                                title: 'Error!',
                                text: 'Failed to create holidays.'
                            });
                        }
                    });                                    
                }
    ...
    <script>
        document.addEventListener('DOMContentLoaded', function() {
            ...
            document.querySelector('.reset-btn').addEventListener('click', function (e) {
                e.preventDefault();
                window.location = '{{ route('role.holidays.index', ['role' => Str::slug(Auth::user()->getRoleNames()->first())]) }}';
            });
        });
    </script>
@endsection