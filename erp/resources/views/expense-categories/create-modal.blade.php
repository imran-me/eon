<div id="createModal" class="modal fixed inset-0 z-50 hidden">
    ...
    <div class="modal-container bg-white w-11/12 md:max-w-lg mx-auto rounded-xl shadow-2xl z-50 overflow-y-auto pointer-events-auto">
        ...
        {{-- Modal header --}}
        <div class="flex items-center justify-between px-6 py-4 border-b border-gray-100 bg-gradient-to-r from-blue-600 to-blue-500 rounded-t-xl">
            <div class="flex items-center gap-3">
                <div class="w-8 h-8 rounded-lg flex items-center justify-center" style="background:rgba(255,255,255,0.2)">
                    <i class="fas fa-tags text-sm" style="color:#fff"></i>
                </div>
                <h3 class="font-semibold text-lg" style="color:#fff">Add New Category</h3>
            </div>
        ...
        <div class="px-6 py-5">
            <form class="closest" id="createForm" action="{{ route('role.expense-categories.store', ['role' => Str::slug(Auth::user()->getRoleNames()->first())]) }}" method="POST">
                ...
                <div class="space-y-4">
                    ...
                    <div>
                        <label for="create_company_id" class="block text-sm font-semibold text-gray-700 mb-1.5">
                            Company
                        </label>
                        {{-- Blank is not "unfilled" here, it is SHARED — the normal
                             case, so it is named and selected rather than sitting
                             behind a "— Select Company —" that reads as an omission. --}}
                        <select id="create_company_id" name="company_id" class="form-select w-full px-3 py-2 border border-gray-300 rounded-lg focus:outline-none focus:ring-2 focus:ring-blue-500 focus:border-blue-500 select2 text-sm" style="width: 100%">
                        ...
                        <p class="text-xs text-gray-500 mt-1.5">
                            Leave as <span class="font-medium">All companies</span> unless this is something only one company ever spends on.
                        </p>
                    </div>
                    ...
                    <div>
                        ...
                        <input type="text" id="create_name" name="name"
                            class="form-input w-full px-3 py-2 border border-gray-300 rounded-lg focus:outline-none focus:ring-2 focus:ring-blue-500 focus:border-blue-500 text-sm"
                    ...
                    <div>
                        <label for="create_account_id" class="block text-sm font-semibold text-gray-700 mb-1.5">
                            Posts To Account
                        </label>
                        <select id="create_account_id" name="account_id"
                    ...
                    <div>
                        ...
                        <textarea id="create_description" name="description" rows="3" placeholder="Optional description…"
                            class="w-full rounded-lg border border-gray-300 focus:border-blue-500 focus:ring-2 focus:ring-blue-100 px-3 py-2 text-sm text-gray-700 outline-none resize-none transition duration-150"></textarea>
                    </div>

                    <div class="flex items-center gap-3 py-1">
                        <label class="relative inline-flex items-center cursor-pointer">
                            <input type="checkbox" name="status" id="create_status" class="sr-only peer" checked>
                            <div class="w-10 h-5 bg-gray-300 peer-checked:bg-blue-500 rounded-full transition-colors duration-200 after:content-[''] after:absolute after:top-0.5 after:left-0.5 after:bg-white after:rounded-full after:h-4 after:w-4 after:transition-all peer-checked:after:translate-x-5"></div>
                        </label>
        ...
        <div class="flex justify-end gap-3 px-6 py-4 bg-gray-50 rounded-b-xl border-t border-gray-100">
            ...
            <button id="createSubmit" type="button" class="px-5 py-2 text-sm font-semibold text-white bg-blue-600 rounded-lg hover:bg-blue-700 transition duration-150 inline-flex items-center gap-2">
