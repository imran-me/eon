...
@section('meta-information')
    <title>Manage New Promotion</title>
@endsection
@section('css')
...
<div class="manage-user-container">
    <div class="form-card">
        ...
        <div class="form-body">
            ...
            <form action="{{ route('role.promotions.store', ['role' => Str::slug(Auth::user()->getRoleNames()->first())]) }}" method="POST" class="promotion-form">
                @csrf
                ...
                <div>
                    ...
                    <select name="user_id" class="form-control select2" onchange="manageProfile()" required>
                        ...
                        @foreach($users as $user)
                            <option value="{{ $user->id }}" data-has_profile="{{ empty($user->profile) ? 0 : 1 }}">{{ $user->name }}</option>
                        @endforeach
                    </select>
                    @error('user_id') <small class="text-red-600">{{ $message }}</small> @enderror
                </div>
                   
                <div class="has-no-profile" style="display: none">
                ...
                <div>
                    ...
                    <textarea name="reason" rows="3" class="form-control" placeholder="Optional reason"></textarea>
                </div>

                {{-- Submit --}}
                <div class="flex justify-end">
                    <button class="btn btn-primary btn-submit">Create Promotion</button>
                </div>
            </form>
