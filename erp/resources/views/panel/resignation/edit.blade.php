...
<div class="manage-user-container">
    <div class="form-card">
        <div class="form-header">
            ...
            <p>Update the employee's resignation details.</p>
        </div>
        ...
        <div class="form-body">
            ...
            <form action="{{ route('role.resignations.update', ['role' => Str::slug(Auth::user()->getRoleNames()->first()), 'resignation' => $resignation->id]) }}" method="POST" enctype="multipart/form-data">
                @csrf
                @method('PUT')
                ...
                <div class="form-section">
                    ...
                    <div class="form-grid">
                        <div class="form-group">
                            ...
                            <select class="select2" id="employee_id" disabled>
                                @foreach($employees as $item)
                                    <option value="{{ $item->id }}" {{ old('employee_id', $resignation->employee_id) == $item->id ? 'selected' : '' }}>{{ ucfirst($item->name) }}</option>
                                @endforeach
                                @if(!$employees->contains('id', $resignation->employee_id))
                                    <option value="{{ $resignation->employee_id }}" selected>{{ ucfirst($resignation->user->name) }}</option>
                                @endif
                            </select>
                            <input type="hidden" name="employee_id" value="{{ old('employee_id', $resignation->employee_id) }}">
                        </div>
                        <div class="form-group">
                            <label for="resigned_date">Resigned Date*</label>
                            <input type="date" name="resign_date" id="resigned_date" placeholder="Enter Resigned Date" required value="{{ old('resign_date', \Carbon\Carbon::parse($resignation->resign_date)->format('Y-m-d')) }}">
                        </div>
                        <div class="form-group">
                            <label for="last_working_day">Last Working day *</label>
                            <input type="date" name="last_working_day" id="last_working_day" placeholder="Enter Last Working Day" required value="{{ old('last_working_day', \Carbon\Carbon::parse($resignation->last_working_day)->format('Y-m-d')) }}">
                        </div>
                        <div class="form-group">
                            <label for="image">Resign Type</label>
                            <select name="resign_type" class="select2" id="resign_type" required>                                
                                <option value="voluntary" {{ old('resign_type', $resignation->resign_type) == 'voluntary' ? 'selected' : '' }}>Voluntary</option>
                                <option value="termination" {{ old('resign_type', $resignation->resign_type) == 'termination' ? 'selected' : '' }}>Termination</option>
                                <option value="abscond" {{ old('resign_type', $resignation->resign_type) == 'abscond' ? 'selected' : '' }}>Abscond</option>
                            </select>
                        </div>
                        <div class="form-group">
                            <label for="reason">Reason  *</label>
                            <select name="reason" id="reason" required>
                                <option value="personal" {{ old('reason', $resignation->reason) == 'personal' ? 'selected' : '' }}>Personal</option>
                                <option value="health" {{ old('reason', $resignation->reason) == 'health' ? 'selected' : '' }}>Health</option>
                                <option value="better_opportunity" {{ old('reason', $resignation->reason) == 'better_opportunity' ? 'selected' : '' }}>Better Opportunity</option>
                                <option value="company_policy_issue" {{ old('reason', $resignation->reason) == 'company_policy_issue' ? 'selected' : '' }}>Company Policy Issue</option>
                                <option value="career_change" {{ old('reason', $resignation->reason) == 'career_change' ? 'selected' : '' }}>Career Change</option>
                                <option value="relocation" {{ old('reason', $resignation->reason) == 'relocation' ? 'selected' : '' }}>Relocation</option>
                                <option value="other" {{ old('reason', $resignation->reason) == 'other' ? 'selected' : '' }}>Other</option>
                            </select>
                        </div>
                        <div class="form-group">
                            <label for="notice_period_days">Notice Period Days *</label>
                            <input type="number" name="notice_period_days" id="notice_period_days" placeholder="Notice Period Days" value="{{ old('notice_period_days', $resignation->notice_period_days) }}" required>
                        </div>
                        <div class="form-group">
                            <label for="approved_at">Approved At</label>
                            <input type="date" name="approved_at" id="approved_at" placeholder="Enter Approved At" value="{{ old('approved_at', \Carbon\Carbon::parse($resignation->approved_at)->format('Y-m-d')) }}">
                        </div>
                        <div class="form-group">
                            <label for="exit_note">Exit Note</label>
                            <textarea type="text" name="exit_note" id="exit_note" placeholder="Enter Exit Note">{{ old('exit_note', $resignation->exit_note) }}</textarea>
                        </div>
                        <div class="form-group">
                            ...
                            <select name="status" class="select2" id="status" required>
                                ...
                                <option value="approved" {{ old('status', $resignation->status) == 'approved' ? 'selected' : '' }}>Approved</option>
                ...
                <div class="form-section">
                    ...
                    @if($resignation->attachments->isNotEmpty())
                        <label class="block text-gray-600 text-xs font-medium mb-1">Existing Attachments</label>
                        <div id="existing_attachments" class="space-y-1 mb-3">
                            @foreach($resignation->attachments as $att)
                                <div class="flex items-center justify-between bg-gray-50 p-2 rounded-md mb-1" data-attachment-row="{{ $att->id }}">
                                    <a href="{{ asset($att->file_path) }}" target="_blank" class="text-blue-600 hover:underline text-sm">
                                        <i class="fas fa-file mr-1"></i>{{ $att->file_name }}
                                    </a>
                                    <button type="button" class="delete-attachment btn btn-danger px-2 py-1 bg-red-500 text-white rounded-md hover:bg-red-600 text-xs"
                    ...
                    <label class="block text-gray-600 text-xs font-medium mb-1">Add New Files</label>
                    <div id="edit_attachments_wrapper" class="space-y-1"></div>
                    <button type="button" id="edit_add_attachment" class="mt-1 text-xs text-blue-600 hover:underline">
...
@section('js')
    ...
    <script>
        ...
        $(document).on('click', '.delete-attachment', function() {
            ...
            const resignationId = '{{ $resignation->id }}';
            const row = $(this).closest('[data-attachment-row]');
            ...
            Swal.fire({
                ...
                if (result.isConfirmed) {
                    $.ajax({
                        url: `/${role}/resignations/${resignationId}/attachment/${attachmentId}`,
                        method: 'DELETE',
