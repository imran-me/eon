...
@section('main-content')
                    ...
                    <table class="table table-hover min-w-full divide-y divide-gray-200">
                        <thead class="bg-gray-50">
                            <tr>
                                ...
                                <th style="width: 10%" class="px-6 py-3 text-left text-xs font-medium text-gray-500 uppercase tracking-wider">Device Location</th>
                                <th style="width: 10%" class="px-6 py-3 text-left text-xs font-medium text-gray-500 uppercase tracking-wider">Device Serial Number</th>
                                    ...
                                    <td class="px-6 py-4 whitespace-nowrap">
                                        <div class="btn-group btn-group-sm flex space-x-1">
                                            <button class="btn btn-outline-primary edit-btn border border-blue-500 text-blue-500 hover:bg-blue-500 hover:text-white px-3 py-1 rounded-md transition duration-200"
                                                data-item_name="{{ $value->name }}" data-item_device_location="{{ $value->device_location }}" data-item_device_serial_no="{{ $value->device_serial_no }}" data-item_status="{{ $value->is_active }}" data-item_id="{{ $value->id }}"
                                                data-action="{{ route('role.device-settings.update', ['role' => Str::slug(Auth::user()->getRoleNames()->first()), 'device_setting' => $value->id]) }}" title="Edit Item">
    ...
    @include('device.create-modal')
    @include('device.edit-modal')
    @include('device.delete-modal')
    ...
    <script>
        $(document).ready(function() {
            ...
            $('.edit-btn').click(function() {
                ...
                const item_device_location = $(this).data('item_device_location');
                const item_device_serial_no = $(this).data('item_device_serial_no');
                const item_status = $(this).data('item_status');
                // Set values in the edit form
                $('#editItemId').val(item_id);
                $('#edit_name').val(item_name);
                $('#edit_device_location').val(item_device_location);
                $('#edit_device_serial_no').val(item_device_serial_no);
        ...
        function validateCreateForm() {
            ...
            if (!$('#create_device_serial_no').val().trim()) {
                $('#create_device_serial_no').next('.error-message').removeClass('hidden');
                $('#create_device_serial_no').addClass('border-red-500');
                isValid = false;
            }
