...
<title>File Sales Invoice {{ $sale->invoice_number }} — {{ optional($company)->name }}</title>
...
<div class="card">
    ...
    {{-- Label bar ── --}}
    <div class="label-bar">Contract File Sales — Invoice</div>
    ...
    <div class="meta-row">
        <div class="bill-to">
            ...
            <p style="font-family:'Space Mono',monospace;font-size:11px;">{{ $sale->client->phone }}</p>
            @endif
            @if(optional($sale->country)->name)
            <p>Country: {{ $sale->country->name }}</p>
            @endif
        </div>
        <div>
            <table class="meta-tbl">
                <tr><td>Invoice No</td><td class="nf">{{ $sale->invoice_number }}</td></tr>
                <tr><td>Date</td><td class="nf">{{ $sale->sale_date?->format('d M Y') ?? now()->format('d M Y') }}</td></tr>
                <tr><td>Files</td><td>{{ $items->count() }}</td></tr>
                <tr>
                    <td>Status</td>
                    <td><span class="pill pill-{{ $sale->payment_status }}">{{ ucfirst($sale->payment_status) }}</span></td>
                </tr>
            </table>
        </div>
    </div>
    ...
    <table class="items-table">
        <thead>
            <tr>
                <th width="5%">#</th>
                <th width="34%">Applicant / File Details</th>
                <th width="18%">Country</th>
                <th width="18%">Category</th>
                <th class="r" width="12%">Unit Price</th>
        ...
        <tbody>
            ...
            <tr>
                ...
                <td>
                    <strong>{{ $file?->applicant_name ?? '—' }}</strong>
                    <span class="sub">
                        File: {{ $file?->file_number ?? '—' }}
                        @if($file?->passport_no) &nbsp;|&nbsp; PP: {{ $file->passport_no }}@endif
                    </span>
                </td>
            ...
            @endforeach
            <tr class="total-row">
                <td colspan="4" style="font-family:var(--tf);">Grand Total — {{ $items->count() }} File(s)</td>
                <td></td>
                <td class="r nf">৳{{ number_format($sale->total_amount, 2) }}</td>
            </tr>
        </tbody>
    </table>
    ...
    <div class="bot">
        <div>
            ...
            <div class="nb">
                @php
                    try { $words = ucfirst((new \NumberToWords\NumberToWords())->getNumberTransformer('en')->toWords((int) $sale->total_amount)) . ' Taka Only'; }
                    catch (\Throwable) { $words = '৳' . number_format($sale->total_amount, 2) . ' only'; }
                @endphp
                {{ $words }}
            </div>
            ...
            <div class="nb">{{ $sale->notes }}</div>
            @endif
        </div>
        <div>
            <div class="sh">Amounts:</div>
            <table class="amt-tbl">
                <tr class="sub-total"><td>Sub Total</td><td class="n">৳{{ number_format($sale->total_amount, 2) }}</td></tr>
                <tr class="grand"><td>Total Due</td><td class="n">৳{{ number_format($sale->total_amount, 2) }}</td></tr>
                <tr class="paid"><td>Received</td><td class="n">৳{{ number_format($sale->paid_amount, 2) }}</td></tr>
                @if($sale->due_amount > 0)
                <tr class="bal"><td>Balance</td><td class="n">৳{{ number_format($sale->due_amount, 2) }}</td></tr>
                @else
