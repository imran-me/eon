...
<style>
    ...
    /* Info - Blue (আপনি যেটা খুঁজছিলেন) */
    .bg-info-custom { background-color: #e0f2fe; color: #075985; border: 1px solid #7dd3fc; }

    /* Dark Variants (যেমন bg-green-800 এর জন্য) */
    .bg-green-dark { background-color: #166534; color: #ffffff; }
    .bg-info-dark { background-color: #075985; color: #ffffff; }
    .modal {
...
@section('main-content')
        ...
        <div class="states-table-container">
            <div class="states-table-header bg-blue-600 px-6 py-4 flex justify-between items-center">
                <h2 class="states-table-title text-white text-xl font-semibold" style="color: white">
                    <i class="fas fa-list mr-2"></i>Ticket Purchase List
                </h2>                
                <a href="{{ route('role.ticket-purchase.create', ['role' => Str::slug(Auth::user()->getRoleNames()->first())]) }}" class="btn btn-primary create-new-btn bg-blue-500 hover:bg-blue-600 text-white px-4 py-2 rounded-md transition duration-200">
                    <i class="fas fa-plus mr-2"></i>Add New Ticket Purchase
                </a>
            </div>
                        ...
                        <div class="filter-content">
                            <div class="closest filter-row">                                                                                                                                                                                      
                                <div class="filter-group">
                                    <label for="passport_holder_id">Passport Holders</label>
                                    <select id="passport_holder_id" name="passport_holder_id" class="form-control select2" style="width: 100%">
                                        <option value="">All</option>                                        
                                        @foreach ($passport_holders as $passport_holder)
                                            <option value="{{ $passport_holder->id }}" {{ $passport_holder->id == request('passport_holder_id') ? 'selected' : '' }}>{{ $passport_holder->name }}</option>                                            
                                        @endforeach
                                    </select>
                                </div>                                
                                {{-- <div class="filter-group">
                                ...
                                <div class="filter-group">
                                    ...
                                    <select id="ticket_id" name="ticket_id" class="form-control select2" style="width: 100%">
                                        ...
                                        @foreach ($tickets as $ticket)
                                            <option value="{{ $ticket->id }}" {{ $ticket->id == request('ticket_id') ? 'selected' : '' }}>{{ $ticket->name }}</option>                                            
                                        @endforeach
                                    </select>
                                </div>         
                                <div class="filter-group">
                                    <label for="vendor_id">Vendor</label>
                                    <select id="vendor_id" name="vendor_id" class="form-control select2" style="width: 100%">
                                        <option value="">All</option>
                                ...
                                <div class="filter-group">
                                    <label for="portal_id">Portals</label>
                                    <select id="portal_id" name="portal_id" class="form-control select2" style="width: 100%">
                                        <option value="">All</option>
                                ...
                                <div class="filter-group">
                                    ...
                                    <select id="ticket_type" name="ticket_type" class="form-control select2" style="width: 100%">
                                        <option value="">All</option>
                                        <option value="air" {{ request('ticket_type') == 'air' ? 'selected' : '' }}>Air</option>
                                        <option value="bus" {{ request('ticket_type') == 'bus' ? 'selected' : '' }}>Bus</option>
                                        <option value="train" {{ request('ticket_type') == 'train' ? 'selected' : '' }}>Train</option>
                                        <option value="other" {{ request('ticket_type') == 'other' ? 'selected' : '' }}>Other</option>
                                    </select>
                                </div>
                                <div class="filter-group">
                                    <label for="trip_type">Trip Type</label>
                                    <select id="trip_type" name="trip_type" class="form-control select2" style="width: 100%">
                                        <option value="">All</option>
                                        <option value="one-way" {{ request('trip_type') == 'one-way' ? 'selected' : '' }}>One-way</option>
                                        <option value="two-way" {{ request('trip_type') == 'two-way' ? 'selected' : '' }}>Two-way</option>
                                    </select>
                                </div>                                       
                                <div class="filter-group">
                                ...
                                <div class="filter-group">
                                    ...
                                    <input type="text" name="ticket_no" value="{{ request('ticket_no') }}" id="ticket_no" class="form-control" placeholder="Search...">
                                </div>
                            </div>
                    ...
                    <table class="table table-hover min-w-full divide-y divide-gray-200">
                        <thead class="bg-gray-50">
                            <tr>
                                ...
                                <th class="px-6 py-3 text-left text-xs font-medium text-gray-500 uppercase tracking-wider">Passport Holder</th>
                            ...
                            @forelse ($datas as $key => $value)                       
                                <tr>
                                    ...
                                    <td class="px-6 py-4 whitespace-nowrap">
                                        <strong>{{ $value->ticket_no }}</strong> <br>
                                        {{ $value->purchase_date }}
                                    </td>
                                    <td class="px-6 py-4 whitespace-nowrap">{{ $value->passportHolder?->name ?? '-' }}</td>
                                    <td class="px-6 py-4 whitespace-nowrap">{{ $value->vendor?->name ?? '-' }}</td>
                                    <td class="px-6 py-4 whitespace-nowrap">
                                        <div class="badge bg-confirm" style="display: inline-block;">By {{ ucfirst($value->ticket_type) }}</div>
                                        {{ $value->ticket?->title ?? '-' }}
                                    </td>
                                    <td class="px-6 py-4 whitespace-nowrap">
                                        <div class="badge bg-info-custom">{{ ucfirst($value->trip_type) }}</div>
                                    </td>
                                    <td class="px-6 py-4 whitespace-nowrap">
                                        Paid: <span style="color: rgb(37, 151, 37)">{{ $value->paid_amount }}</span> <br>
                                        Due: <span style="color: rgb(255, 81, 81)">{{ $value->due_amount }}</span> <br>
                                        Total: <strong>{{ $value->amount }}</strong>
                                    </td>
                                    <td class="px-6 py-4 whitespace-nowrap">
                                        @php
                                            $statusColors = [
                                            ...
                                            $statusClass = $statusColors[$value->status] ?? 'bg-draft';
                                        @endphp
                                    ...
                                    <td class="px-6 py-4 whitespace-nowrap">
                                        <div class="btn-group btn-group-sm flex space-x-1">
                                            ...
                                            <a target="_blank" href="{{ route('role.ticket-purchase.show', ['role' => Str::slug(Auth::user()->getRoleNames()->first()), 'ticket_purchase' => $value->id]) }}"
                                                class="btn btn-outline-primary border border-indigo-500 text-indigo-500 hover:bg-indigo-500 hover:text-white px-3 py-1 rounded-md transition duration-200"
                                                title="View Invoice">
                                                <i class="fas fa-print"></i>
                                            </a>
                                            
                                            {{-- Edit: not allowed if cancelled --}}
                                            @if(!in_array($value->status, ['confirm', 'cancelled']))
                                                <a href="{{ route('role.ticket-purchase.edit', ['role' => Str::slug(Auth::user()->getRoleNames()->first()), 'ticket_purchase' => $value->id]) }}"
                                            ...
                                            @else
                                                ...
                                                <span class="border border-gray-300 text-gray-300 px-3 py-1 rounded-md cursor-not-allowed" title="Cannot edit a cancelled purchase">
                                                    <i class="fas fa-edit"></i>
                                                </span>
                                            @endif

                                            {{-- Delete: not allowed if confirmed --}}
                                            @if($value->status !== 'confirm')
                                                <button class="btn btn-outline-danger border border-red-500 text-red-500 hover:bg-red-500 hover:text-white px-3 py-1 rounded-md transition duration-200"
                                                        onclick="confirmDelete('{{ $value->id }}', '{{ $value->ticket_no }}')"
                                                        title="Delete">
                                                    <i class="fas fa-trash"></i>
                                                </button>
                                            @else
                                                {{-- Greyed out, non-clickable --}}
                                                <span class="border border-gray-300 text-gray-300 px-3 py-1 rounded-md cursor-not-allowed" title="Cannot delete a confirmed purchase">
                                                    <i class="fas fa-trash"></i>
                                                </span>
                                            @endif
    ...
    @include('dashboard.ticket_purchases.delete-modal')
...
@section('js')
    ...
    <script>
        ...
        document.addEventListener('DOMContentLoaded', function() {
            ...
            // Auto-open filter if any params present
            @php
                $hasFilters = collect(['passport_holder_id','ticket_id','vendor_id','portal_id','ticket_type','trip_type','created_by','status','from_date','to_date','ticket_no'])->contains(fn($k) => request($k));
            @endphp
            @if($hasFilters)
            ...
            document.querySelector('.reset-btn').addEventListener('click', function (e) {
                e.preventDefault();
                window.location = '{{ route('role.ticket-purchase.index', ['role' => Str::slug(Auth::user()->getRoleNames()->first())]) }}';
            });
        });
    </script>
@endsection