...
@section('css')
    ...
    @include('dashboard.ticket_purchases.create-css')
@endsection
        ...
        <div class="states-table-container">
            ...
            <div class="states-table-content" style="padding: 15px;">
                <div class="common-modal-body modal-body overflow-y-auto mt-2">
                    <form class="closest" id="purchaseCreateForm" action="{{ route('role.ticket-purchase.store', ['role' => Str::slug(Auth::user()->getRoleNames()->first())]) }}" method="POST" enctype="multipart/form-data">
                        @csrf
                        ...
                        <div class="grid grid-cols-1 md:grid-cols-4 gap-4 pt-4" style="justify-content: center">   
                            <div class="mb-2">
                                ...
                                <select id="passport_holder_id" required name="passport_holder_id" class="form-select w-full px-3 py-2 border border-gray-300 rounded-md focus:outline-none focus:ring-2 focus:ring-blue-500 select2" style="width: 100%">                                      
                                    @foreach ($passport_holders as $passport_holder)
                                        <option value="{{ $passport_holder->id }}">{{ $passport_holder->name }}</option>                                            
                                    @endforeach
                                </select>
                            </div>
                            <div class="mb-2">
                            ...
                            <div class="mb-2">
                                ...
                                <input type="text" id="currency" required value="৳" name="currency" class="form-input w-full px-3 py-2 border border-gray-300 rounded-md focus:outline-none focus:ring-2 focus:ring-blue-500">
                            </div>
                            <div class="mb-2">
                            ...
                            <div class="mb-2">
                                ...
                                <input type="text" name="ticket_no" required value="{{ $ticketNo }}" class="form-input w-full px-3 py-2 border border-gray-300 rounded-md focus:outline-none focus:ring-2 focus:ring-blue-500">
                            </div>                            
                            <div class="mb-2">
                                        ...
                                        <div class="grid grid-cols-1 md:grid-cols-12 gap-4">
                                            <div class="md:col-span-12">
                                                ...
                                                <select id="isProductSelected" required name="ticket_id" onchange="selectProduct(this)" class="select2 form-select w-full px-3 py-2 border border-gray-300 rounded-md focus:outline-none focus:ring-2 focus:ring-blue-500">
                                                    ...
                                                    @foreach ($tickets as $ticket)
                                                        <option value="{{ $ticket->id }}" data-portal_id="{{ $ticket->portal_id }}" data-vendor_id="{{ $ticket->vendor_id }}" data-price="{{ $ticket->price }}">{{ $ticket->title }}</option>
                                                    @endforeach
                                                </select>
                                            </div>                                            
                                            ...
                                            <div class="md:col-span-4">
                                                ...
                                                <div class="relative">
                                                    ...
                                                    <input id="selectsubtotal" onkeyup="applyGrandTotal(this)" type="text" name="price" class="form-input w-full pl-8 pr-3 py-2 border border-gray-300 rounded-md bg-gray-50" value="0.00">
                                                </div>
                                            </div>                                        
                                        </div>
                                    </div>                                                        
                                ...
                                <div class="calculation-card p-5" style="width: 40%;">
                                    ...
                                    <div class="space-y-4">
                                        <div>
                                            ...
                                            <div class="relative">
                                                ...
                                                <input type="number" onkeyup="calculatePaidAmount(this)" value="0.00" step="0.01" id="paid_amount" name="paid_amount" class="form-input w-full pl-8 pr-3 py-2 border border-gray-300 rounded-md focus:outline-none focus:ring-2 focus:ring-blue-500" placeholder="0.00" style="background: white">                                                
                                            </div>
                                        </div>                                                                             
                                    ...
                                    <div class="space-y-4">                                        
                                        <div class="border-t border-gray-300 pt-3 flex justify-between items-center" style="border-top-width: 2px;">
                                            ...
                                            <span class="amount-highlight">৳ <span class="paid_amount">0.00</span></span>                                            
                                        </div>
                                        <div class="flex justify-between items-center"> --}}
                                            <span class="text-gray-800 font-semibold">Total Due:</span>
                                            <span class="amount-highlight">৳ <span class="due_amount">0.00</span></span>
                                            <input type="hidden" name="due_amount" id="due_amount" value="0">
                                        </div>
                                        <div class="flex justify-between items-center">
                                            <span class="text-gray-800 font-semibold">Total Amount:</span>
                                            <span class="text-xl font-bold text-blue-600">৳ <span class="amount">0.00</span></span>
...
@section('js')
    ...
    <script>
        ...
        $('#createpurchaseBtn').click(function(e) {
            ...
            if (!$('#isProductSelected').val()) {
                Swal.fire({ icon: 'error', title: 'Error!', text: 'Please Select a ticket!' });
                return;
            }
            ...
            $.ajax({
                ...
                success: function (response) {
                    if (response.success) {
                        Swal.fire({ icon: 'success', title: 'Done', text: 'Purchase created successfully!' });
                        setTimeout(() => { window.location.href = "{{ url('admin/ticket-purchase') }}"; }, 800);
                    } else {
                ...
                error: function (xhr) {
                    ...
                    $btn.prop('disabled', false).html('Submit');
                    Swal.fire({ icon: 'error', title: 'Error!', text: 'Failed to create purchase.' });
                }
            });
        });
        ...
        function selectProduct(obj) {      
            ...
            $('#amount, #due_amount').val(item_price ? item_price : 0);
            $('.amount, .due_amount').text(item_price ? item_price : 0);
        ...
        function calculatePaidAmount() {
            ...
            var due = (total_amount - paid_amount).toFixed(2);
            $('.due_amount').text(due);
            $('#due_amount').val(due);
            ...
            if (total_amount > 0) {
                var status;
                if (paid_amount <= 0) status = 'due';
                else if (paid_amount >= total_amount) status = 'paid';
                else status = 'partial';
