...
<div class="min-h-screen bg-gray-50 p-6">
    ...
    <div class="bg-white rounded-xl shadow-sm border border-gray-100">
        ...
        <div class="flex flex-wrap items-center gap-3 p-4 border-b border-gray-100">
            {{-- Search --}}
            <form method="GET" action="{{ route('role.customer.index', ['role' => Str::slug(Auth::user()->getRoleNames()->first())]) }}"
                ...
                <div class="relative min-w-[260px]">
                    ...
                    <input type="text" name="search" value="{{ request('search') }}"
                           ...
                           class="w-full pl-9 pr-4 py-2 text-sm border border-gray-200 rounded-lg focus:outline-none focus:ring-2 focus:ring-amber-500 focus:border-transparent bg-gray-50">
                </div>
                <button type="submit"
            ...
            <div class="flex items-center gap-2 ml-auto">
                <a href="{{ route('role.customer.export.excel', array_merge(['role' => Str::slug(Auth::user()->getRoleNames()->first())], request()->only('search'))) }}"
                   class="flex items-center gap-1.5 px-4 py-2 text-sm font-medium text-gray-600 border border-gray-200 rounded-lg hover:bg-gray-50 transition">
                ...
                <a href="{{ route('role.customer.export.pdf', array_merge(['role' => Str::slug(Auth::user()->getRoleNames()->first())], request()->only('search'))) }}"
                   class="flex items-center gap-1.5 px-4 py-2 text-sm font-medium text-gray-600 border border-gray-200 rounded-lg hover:bg-gray-50 transition">
                ...
                <a href="{{ route('role.customer.create', ['role' => Str::slug(Auth::user()->getRoleNames()->first())]) }}"
                   class="flex items-center gap-1.5 px-4 py-2 text-sm font-semibold text-white bg-amber-600 rounded-lg hover:bg-amber-700 transition shadow-sm">
                    <svg class="w-4 h-4" fill="none" stroke="currentColor" viewBox="0 0 24 24">
            ...
            <table class="min-w-full text-sm">
                <thead>
                    <tr class="bg-gray-50 border-b border-gray-100 text-xs font-semibold text-gray-500 uppercase tracking-wider">
                        ...
                        <th class="px-5 py-3 text-left">Balance / Receivable</th>
                ...
                <tbody class="divide-y divide-gray-50">
                    @forelse($customers as $customer)
                        @php
                            $iteration  = $loop->iteration + ($customers->currentPage() - 1) * $customers->perPage();
                            $customerId = 'EP-CU-' . str_pad($customer->id, 3, '0', STR_PAD_LEFT);
                        @endphp
                        <tr class="hover:bg-amber-50/30 transition-colors group">

                            {{-- Customer ID --}}
                            <td class="px-5 py-3.5">
                                <span class="font-mono text-xs font-semibold text-amber-600 bg-amber-50 px-2 py-1 rounded">
                                    {{ $customerId }}
                                </span>
                            </td>
                            ...
                            <td class="px-5 py-3.5">
                                <div class="flex items-center gap-3">
                                    ...
                                    <div>
                                        <p class="font-semibold text-gray-800 leading-tight">{{ $customer->name }}</p>
                            ...
                            <td class="px-5 py-3.5">
                                <div>
                                    ...
                                    @if($customer->contact_person)
                                        <p class="text-xs text-gray-400 mt-0.5">{{ $customer->contact_person }}</p>
                                    @endif
                                </div>
                            </td>
                            ...
                            <td class="px-5 py-3.5">
                                @if($customer->address)
                                    <span class="text-gray-600 text-xs">{{ Str::limit($customer->address, 30) }}</span>
                                @else
                                    <span class="text-gray-300 text-xs italic">—</span>
                                @endif
                            </td>
                            ...
                            {{-- Balance / Payable --}}
                            @php $ledger = $balances[$customer->id] ?? null; @endphp
                            <td class="px-5 py-3.5">
                                <a href="{{ route('role.party-statement.index', ['role' => Str::slug(Auth::user()->getRoleNames()->first()), 'party_id' => $customer->id]) }}"
                            ...
                            <td class="px-5 py-3.5">
                                <div class="flex items-center justify-center gap-1">
                                    {{-- Eye / View --}}
                                    <a href="{{ route('role.customer.edit', ['role' => Str::slug(Auth::user()->getRoleNames()->first()), 'customer' => $customer->id]) }}"
                                       class="w-8 h-8 flex items-center justify-center rounded-lg text-amber-500 bg-amber-50 hover:bg-amber-100 transition"
                                       title="View / Edit">
                                        <svg class="w-4 h-4" fill="none" stroke="currentColor" viewBox="0 0 24 24">
                                            <path stroke-linecap="round" stroke-linejoin="round" stroke-width="2"
                                    ...
                                    {{-- Edit (Pencil) --}}
                                    <a href="{{ route('role.customer.edit', ['role' => Str::slug(Auth::user()->getRoleNames()->first()), 'customer' => $customer->id]) }}"
                                       class="w-8 h-8 flex items-center justify-center rounded-lg text-orange-500 bg-orange-50 hover:bg-orange-100 transition"
                                       title="Edit">
                                        <svg class="w-4 h-4" fill="none" stroke="currentColor" viewBox="0 0 24 24">
                                            <path stroke-linecap="round" stroke-linejoin="round" stroke-width="2"
                                    ...
                                    <button type="button"
                                            ...
                                            data-delete-name="{{ e($customer->name) }}"
        ...
        <div class="flex items-center justify-between px-5 py-4 border-t border-gray-100">
            <p class="text-sm text-gray-500">
                Showing
                <span class="font-semibold text-gray-700">{{ $customers->firstItem() ?? 0 }}</span>–<span class="font-semibold text-gray-700">{{ $customers->lastItem() ?? 0 }}</span>
                of
                <span class="font-semibold text-gray-700">{{ $customers->total() }}</span>
                customers
            </p>
...
<div id="deleteModal" class="fixed inset-0 bg-black/40 backdrop-blur-sm hidden items-center justify-center z-50">
    <div class="bg-white rounded-2xl p-6 w-96 max-w-full shadow-2xl">
        <div class="flex items-center gap-3 mb-4">
            ...
            <h3 class="text-base font-semibold text-gray-800">Delete Customer</h3>
        </div>
        <p id="deleteMessage" class="text-sm text-gray-600 mb-6">Are you sure you want to delete this customer? This action cannot be undone.</p>
...
<div id="confirmModal" class="fixed inset-0 bg-black/40 backdrop-blur-sm hidden items-center justify-center z-50">
    <div class="bg-white rounded-2xl p-6 w-96 max-w-full shadow-2xl animate-in">
        ...
        <p id="confirmMessage" class="text-sm text-gray-600 mb-6 pl-13">Are you sure you want to change the customer status?</p>
        <div class="flex justify-end gap-3">
            ...
            <form id="confirmForm" method="POST" class="inline">
                ...
                @method('PUT')
                <button type="submit"
                        class="px-4 py-2 text-sm font-semibold text-white bg-amber-600 rounded-lg hover:bg-amber-700 transition cursor-pointer shadow-sm">
                    Yes, Confirm
                </button>
            </form>
        </div>
...
<script>
    function openConfirmModal(button) {
        ...
        const currentStatus  = button.getAttribute('data-customer-status');
        ...
        form.action = `/{{ Str::slug(Auth::user()->getRoleNames()->first()) }}/customer/${customerId}/toggle-status`;
    ...
    function openDeleteModal(btn) {
        const customerId   = btn.getAttribute('data-delete-id');
        const customerName = btn.getAttribute('data-delete-name');
        document.getElementById('deleteMessage').textContent =
            `Are you sure you want to delete "${customerName}"? This action cannot be undone.`;
        document.getElementById('deleteForm').action =
            `/{{ Str::slug(Auth::user()->getRoleNames()->first()) }}/customer/${customerId}`;
