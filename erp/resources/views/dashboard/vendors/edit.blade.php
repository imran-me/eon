...
<div class="min-h-screen bg-gray-50 p-6">
    ...
    <div class="flex items-center justify-between mb-6">
        <div>
            ...
            <p class="text-sm text-gray-500 mt-0.5">Epal Group ERP &rsaquo; Vendor Management &rsaquo; {{ $vendor->name }}</p>
        </div>
    ...
    <form action="{{ route('role.vendor.update', ['role' => Str::slug(Auth::user()->getRoleNames()->first()), 'vendor' => $vendor->id]) }}"
          method="POST">
        @csrf
        @method('PUT')
        ...
        <div class="grid grid-cols-1 lg:grid-cols-3 gap-6">
            ...
            <div class="lg:col-span-2 space-y-5">
                    ...
                    <div class="p-6 grid grid-cols-1 sm:grid-cols-2 gap-5">
                        ...
                        <div class="sm:col-span-2">
                            ...
                            <textarea name="address" id="address" rows="3"
                                      ...
                                      class="w-full px-4 py-2.5 text-sm rounded-lg border focus:outline-none focus:ring-2 focus:ring-blue-500 focus:border-transparent transition resize-none {{ $errors->has('address') ? 'border-red-400 bg-red-50' : 'border-gray-200' }}">{{ old('address', $vendor->address) }}</textarea>
                            @error('address')
                                <p class="text-xs text-red-600 mt-1.5">{{ $message }}</p>
                            @enderror
                        </div>
                ...
                {{-- Activity Log Card --}}
                @php $logs = $vendor->logs()->latest()->get(); @endphp
                @if($logs->isNotEmpty())
                <div class="bg-white rounded-xl shadow-sm border border-gray-100 overflow-hidden">
                    <div class="flex items-center gap-3 px-6 py-4 border-b border-gray-100">
                                ...
                                <div class="flex items-start justify-between gap-4">
                                    ...
                                    <div class="flex flex-col items-end gap-2 flex-shrink-0">
                                        ...
                                        @if($log->action === 'updated' && $log->before)
                                            <form action="{{ route('role.vendor.restore', ['role' => Str::slug(Auth::user()->getRoleNames()->first()), 'log' => $log->id]) }}"
                                                  method="POST"
            ...
            <div class="space-y-5">
                ...
                {{-- Vendor Profile Card --}}
                <div class="bg-white rounded-xl shadow-sm border border-gray-100 overflow-hidden">
                    <div class="bg-gradient-to-br from-blue-500 to-indigo-600 px-6 py-8 text-center">
                        ...
                        <h3 class="text-white font-semibold text-base leading-tight">{{ $vendor->name }}</h3>
                        <p class="text-blue-100 text-xs mt-1">{{ $vendor->email }}</p>
                    </div>
                    <div class="px-6 py-4 space-y-3">
                        <div class="flex items-center justify-between text-xs">
                            ...
                            <span class="font-mono font-semibold text-blue-600 bg-blue-50 px-2 py-0.5 rounded">
                                EP-VN-{{ str_pad($vendor->id, 3, '0', STR_PAD_LEFT) }}
                            </span>
                        </div>
                        <div class="flex items-center justify-between text-xs">
                        ...
                        <div class="flex items-center justify-between text-xs">
                            ...
                            <span class="font-medium text-gray-700">{{ $vendor->phone }}</span>
                        </div>
                        @endif
                    </div>
                </div>
                ...
                <div class="bg-white rounded-xl shadow-sm border border-gray-100 overflow-hidden sticky top-6">
                    <div class="flex items-center gap-3 px-6 py-4 border-b border-gray-100">
                        ...
                        <h2 class="text-sm font-semibold text-gray-700">Update Vendor</h2>
                    </div>
