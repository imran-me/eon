...
<div class="min-h-screen bg-gray-50 p-6">
    ...
    <div class="flex items-center justify-between mb-6">
        <div>
            ...
            <p class="text-sm text-gray-500 mt-0.5">Epal Group ERP &rsaquo; Agent Management &rsaquo; {{ $agent->name }}</p>
        </div>
    ...
    <form action="{{ route('role.agent.update', ['role' => Str::slug(Auth::user()->getRoleNames()->first()), 'agent' => $agent->id]) }}"
          method="POST">
        @csrf
        @method('PUT')
                ...
                <div class="bg-white rounded-xl shadow-sm border border-gray-100 overflow-hidden">
                    ...
                    <div class="p-6 grid grid-cols-1 sm:grid-cols-2 gap-5">
                        ...
                        <div>
                            ...
                            <input type="text" name="contact_person" id="contact_person" value="{{ old('contact_person', $agent->contact_person) }}"
                        ...
                        <div class="sm:col-span-2">
                            ...
                            <textarea name="address" id="address" rows="3"
                                      ...
                                      class="w-full px-4 py-2.5 text-sm rounded-lg border focus:outline-none focus:ring-2 focus:ring-violet-500 focus:border-transparent transition resize-none {{ $errors->has('address') ? 'border-red-400 bg-red-50' : 'border-gray-200' }}">{{ old('address', $agent->address) }}</textarea>
                            @error('address')
                                <p class="text-xs text-red-600 mt-1.5">{{ $message }}</p>
                            @enderror
                        </div>
                    ...
                    <div class="p-6 grid grid-cols-1 sm:grid-cols-2 gap-5">
                        ...
                        <div class="relative">
                            ...
                            <input type="password" name="password" id="password"
                                   ...
                                   class="w-full px-4 py-2.5 pr-10 text-sm rounded-lg border focus:outline-none focus:ring-2 focus:ring-violet-500 focus:border-transparent transition {{ $errors->has('password') ? 'border-red-400 bg-red-50' : 'border-gray-200' }}">
                        ...
                        <div class="relative">
                            ...
                            <input type="password" name="password_confirmation" id="password_confirmation"
                                   ...
                                   class="w-full px-4 py-2.5 pr-10 text-sm rounded-lg border border-gray-200 focus:outline-none focus:ring-2 focus:ring-violet-500 focus:border-transparent transition">
            ...
            <div class="space-y-5">
                ...
                <div class="bg-white rounded-xl shadow-sm border border-gray-100 overflow-hidden">
                    <div class="bg-gradient-to-br from-violet-500 to-purple-600 px-6 py-8 text-center">
                        ...
                        <h3 class="text-white font-semibold text-base leading-tight">{{ $agent->name }}</h3>
                        <p class="text-violet-100 text-xs mt-1">{{ $agent->email }}</p>
                    </div>
                    <div class="px-6 py-4 space-y-3">
                        <div class="flex items-center justify-between text-xs">
                            <span class="text-gray-500">Agent ID</span>
                            <span class="font-mono font-semibold text-violet-600 bg-violet-50 px-2 py-0.5 rounded">
                                EP-AG-{{ str_pad($agent->id, 3, '0', STR_PAD_LEFT) }}
                            </span>
                        </div>
                        <div class="flex items-center justify-between text-xs">
                            <span class="text-gray-500">Status</span>
                            @php $status = $agent->status ?? 'inactive'; @endphp
                            @if($status === 'active')
                        ...
                        <div class="flex items-center justify-between text-xs">
                            ...
                            <span class="font-medium text-gray-700">{{ $agent->created_at->format('d M Y') }}</span>
                        </div>
                        @if($agent->phone)
                        <div class="flex items-center justify-between text-xs">
                            <span class="text-gray-500">Phone</span>
                            <span class="font-medium text-gray-700">{{ $agent->phone }}</span>
                        </div>
                        @endif
                        @if($agent->contact_person)
                        <div class="flex items-center justify-between text-xs">
                            <span class="text-gray-500">Contact Person</span>
                            <span class="font-medium text-gray-700">{{ $agent->contact_person }}</span>
                        </div>
                        @endif
                    </div>
                </div>
                ...
                <div class="bg-white rounded-xl shadow-sm border border-gray-100 overflow-hidden sticky top-6">
                    <div class="flex items-center gap-3 px-6 py-4 border-b border-gray-100">
                        ...
                        <h2 class="text-sm font-semibold text-gray-700">Update Agent</h2>
                    </div>
