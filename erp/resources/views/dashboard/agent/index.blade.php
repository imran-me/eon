...
<div class="min-h-screen bg-gray-50 p-6">
    ...
    <div class="bg-white rounded-xl shadow-sm border border-gray-100">
        ...
        {{-- Toolbar --}}
        <div class="flex flex-wrap items-center gap-3 p-4 border-gray-100">
            {{-- Search --}}
            <form method="GET" action="{{ route('role.agent.index', ['role' => Str::slug(Auth::user()->getRoleNames()->first())]) }}"
                ...
                <div class="relative min-w-[260px]">
                    ...
                    <input type="text" name="search" value="{{ request('search') }}"
                           placeholder="Search agent, ID, email..."
            ...
            <div class="flex items-center gap-2 ml-auto">
                <a href="{{ route('role.agent.export.excel', array_merge(['role' => Str::slug(Auth::user()->getRoleNames()->first())], request()->only('search'))) }}"
                   class="flex items-center gap-1.5 px-4 py-2 text-sm font-medium text-gray-600 border border-gray-200 rounded-lg hover:bg-gray-50 transition">
                ...
                <a href="{{ route('role.agent.export.pdf', array_merge(['role' => Str::slug(Auth::user()->getRoleNames()->first())], request()->only('search'))) }}"
                   class="flex items-center gap-1.5 px-4 py-2 text-sm font-medium text-gray-600 border border-gray-200 rounded-lg hover:bg-gray-50 transition">
                ...
                <a href="{{ route('role.agent.create', ['role' => Str::slug(Auth::user()->getRoleNames()->first())]) }}"
        ...
        <div class="overflow-x-auto">
            <table class="min-w-full text-sm">
                <thead>
                    <tr class="bg-gray-50 border-gray-100 text-xs font-semibold text-gray-500 uppercase tracking-wider">
                        <th class="px-5 py-3 text-left">Agent ID</th>
                        <th class="px-5 py-3 text-left">Agent Name</th>
                ...
                <tbody class="divide-y divide-gray-50">
                    @forelse($agents as $agent)
                        @php
                            $agentId = 'EP-AG-' . str_pad($agent->id, 3, '0', STR_PAD_LEFT);
                        ...
                        <tr class="hover:bg-blue-50/30 transition-colors">
                            ...
                            <td class="px-5 py-3.5">
                                <span class="font-mono text-xs font-semibold text-violet-600 bg-violet-50 px-2 py-1 rounded">
                                    {{ $agentId }}
                                </span>
                            </td>
                            ...
                            <td class="px-5 py-3.5">
                                <div class="flex items-center gap-3">
                                    ...
                                    <div>
                                        <p class="font-semibold text-gray-800 leading-tight">{{ $agent->name }}</p>
                                        <p class="text-xs text-gray-400 mt-0.5">{{ $agent->email }}</p>
                                    </div>
                                </div>
                            </td>
                            ...
                            <td class="px-5 py-3.5">
                                <div>
                                    @if($agent->phone)
                                        <p class="text-gray-700 font-medium">{{ $agent->phone }}</p>
                                    @else
                                        <span class="text-gray-300 text-xs italic">—</span>
                                    @endif
                                    @if($agent->contact_person)
                                        <p class="text-xs text-gray-400 mt-0.5">{{ $agent->contact_person }}</p>
                                    @endif
                                </div>
                            </td>
                            ...
                            <td class="px-5 py-3.5">
                                @if($agent->address)
                                    <span class="text-gray-600 text-xs">{{ Str::limit($agent->address, 30) }}</span>
                                @else
                                    <span class="text-gray-300 text-xs italic">—</span>
                                @endif
                            </td>
                            ...
                            {{-- Balance / Payable --}}
                            @php $ledger = $balances[$agent->id] ?? null; @endphp
                            <td class="px-5 py-3.5">
                                <a href="{{ route('role.party-statement.index', ['role' => Str::slug(Auth::user()->getRoleNames()->first()), 'party_id' => $agent->id]) }}"
                            ...
                            <td class="px-5 py-3.5">
                                <div class="flex items-center justify-center gap-1">
                                    ...
                                    {{-- Eye / View --}}
                                    <a href="{{ route('role.agent.edit', ['role' => Str::slug(Auth::user()->getRoleNames()->first()), 'agent' => $agent->id]) }}"
                                    ...
                                    <button type="button"
                                            ...
                                            data-delete-id="{{ $agent->id }}"
                                            data-delete-name="{{ e($agent->name) }}"
        ...
        <div class="flex items-center justify-between px-5 py-4 border-t border-gray-100">
            <p class="text-sm text-gray-500">
                Showing
                <span class="font-semibold text-gray-700">{{ $agents->firstItem() ?? 0 }}</span>–<span class="font-semibold text-gray-700">{{ $agents->lastItem() ?? 0 }}</span>
                of
                <span class="font-semibold text-gray-700">{{ $agents->total() }}</span>
                agents
            </p>
            <div>
...
<div id="deleteModal" class="fixed inset-0 bg-black/40 backdrop-blur-sm hidden items-center justify-center z-50">
    <div class="bg-white rounded-2xl p-6 w-96 max-w-full shadow-2xl">
        <div class="flex items-center gap-3 mb-4">
            ...
            <h3 class="text-base font-semibold text-gray-800">Delete Agent</h3>
        </div>
        <p id="deleteMessage" class="text-sm text-gray-600 mb-6">Are you sure you want to delete this agent? This action cannot be undone.</p>
...
<div id="confirmModal" class="fixed inset-0 bg-black/40 backdrop-blur-sm hidden items-center justify-center z-50">
    <div class="bg-white rounded-2xl p-6 w-96 max-w-full shadow-2xl">
        ...
        <p id="confirmMessage" class="text-sm text-gray-600 mb-6">Are you sure you want to change the agent status?</p>
        <div class="flex justify-end gap-3">
...
<script>
    function openConfirmModal(button) {
        ...
        const currentStatus = button.getAttribute('data-agent-status');
        ...
        form.action = `/{{ Str::slug(Auth::user()->getRoleNames()->first()) }}/agent/${agentId}/toggle-status`;
    ...
    function openDeleteModal(btn) {
        const agentId   = btn.getAttribute('data-delete-id');
        const agentName = btn.getAttribute('data-delete-name');
        document.getElementById('deleteMessage').textContent =
            `Are you sure you want to delete "${agentName}"? This action cannot be undone.`;
        document.getElementById('deleteForm').action =
            `/{{ Str::slug(Auth::user()->getRoleNames()->first()) }}/agent/${agentId}`;
