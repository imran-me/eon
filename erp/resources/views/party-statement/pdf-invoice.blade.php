...
<title>Party Invoice {{ $partyInvoice->invoice_number }} — {{ optional($company)->name }}</title>
...
<style>
    ...
    *, *::before, *::after {
    ...
    .items-table td { padding: 11px 11px; border-bottom: 1px solid var(--bdr); font-size: 12px; }
...
<div class="card">
    ...
    {{-- Label bar ── --}}
    <div class="label-bar">Party Invoice</div>

    {{-- Meta row ── --}}
    <div class="meta-row">
        <div class="bill-to">
            <h4>Bill To:</h4>
            <p><strong>{{ $partyInvoice->user->name }}</strong></p>
            @if($partyInvoice->user->phone)
            <p style="font-family:'Space Mono',monospace;font-size:11px;">{{ $partyInvoice->user->phone }}</p>
            @endif
            @if($partyInvoice->user->email)
            <p>{{ $partyInvoice->user->email }}</p>
            @endif
        </div>
        <div>
            <table class="meta-tbl">
                <tr><td>Invoice No</td><td class="nf">{{ $partyInvoice->invoice_number }}</td></tr>
                <tr><td>Date</td><td class="nf">{{ \Carbon\Carbon::parse($partyInvoice->created_at)->format('d M Y') }}</td></tr>
                @if($partyInvoice->due_date)
                <tr><td>Due Date</td><td class="nf" style="color:#dc2626;">{{ \Carbon\Carbon::parse($partyInvoice->due_date)->format('d M Y') }}</td></tr>
                @endif
                @if($partyInvoice->createdBy)
                <tr><td>Issued By</td><td>{{ $partyInvoice->createdBy->name }}</td></tr>
                @endif
                <tr>
                    <td>Status</td>
                    <td><span style="display:inline-block;padding:2px 10px;border-radius:20px;font-size:10px;font-weight:700;background:#fee2e2;color:#b91c1c;">Due</span></td>
                </tr>
            </table>
        </div>
    </div>
    ...
    <table class="items-table">
        <thead>
            <tr>
                ...
                <th class="r" width="12%">Candidates</th>
        ...
        <tbody>
            <tr>
                <td class="nf">01</td>
                <td>{{ $partyInvoice->service_description }}</td>
                <td>{{ optional($partyInvoice->country)->name ?? '—' }}</td>
                <td class="r nf">{{ $partyInvoice->no_of_candidates ?? '—' }}</td>
                <td class="r nf">৳{{ number_format($partyInvoice->amount, 2) }}</td>
            </tr>
            <tr class="total-row">
    ...
    <div class="bot">
        <div>
            ...
            <div class="nb">
                @php
                    try { $words = ucfirst((new \NumberToWords\NumberToWords())->getNumberTransformer('en')->toWords((int) $partyInvoice->amount)) . ' Taka Only'; }
                    catch (\Throwable) { $words = '৳' . number_format($partyInvoice->amount, 2) . ' only'; }
                @endphp
                {{ $words }}
            </div>

            @if($partyInvoice->notes)
            <div class="sh">Notes / Terms:</div>
            <div class="nb">{{ $partyInvoice->notes }}</div>
            @endif
        </div>
        <div>
            <div class="sh">Amounts:</div>
            <table class="amt-tbl">
                <tr class="sub-total"><td>Sub Total</td><td class="n">৳{{ number_format($partyInvoice->amount, 2) }}</td></tr>
                <tr class="grand"><td>Total Due</td><td class="n">৳{{ number_format($partyInvoice->amount, 2) }}</td></tr>
                <tr class="rcvd"><td>Received</td><td class="n">৳0.00</td></tr>
                <tr class="bal"><td>Balance</td><td class="n">৳{{ number_format($partyInvoice->amount, 2) }}</td></tr>
            </table>
        </div>
    </div>
