...
@section('main-content')
        ...
        <div class="states-table-container">
            <div class="states-table-header bg-blue-600 px-6 py-4 flex justify-between items-center">
                <h2 class="states-table-title text-white text-xl font-semibold" style="color: white">
                    <i class="fas fa-list mr-2"></i>Contract Type List
                </h2>
                @can('create contract')
                <button class="btn btn-primary create-btn bg-blue-500 hover:bg-blue-600 text-white px-4 py-2 rounded-md transition duration-200">
                    <i class="fas fa-plus mr-2"></i>Add Contract Type
                </button>
                @endcan
            </div>
                                    ...
                                    <td class="px-6 py-4 whitespace-nowrap">
                                        <div class="btn-group btn-group-sm flex space-x-1">
                                            ...
                                            <button class="btn btn-outline-primary edit-btn border border-blue-500 text-blue-500 hover:bg-blue-500 hover:text-white px-3 py-1 rounded-md transition duration-200" 
                                                ...
                                                data-action="{{ route('role.contract-types.update', ['role' => Str::slug(Auth::user()->getRoleNames()->first()), 'contract_type' => $value->id]) }}" title="Edit Item">
    ...
    @include('contract-types.create-modal')
    @include('contract-types.edit-modal')
    @include('contract-types.delete-modal')
    ...
    <script>
        $(document).ready(function() {
            ...
            $('.edit-btn').click(function() {
                ...
                const item_active = $(this).data('active');
                
                // Set values in the edit form
                $('#editItemId').val(item_id);
                $('#edit_name').val(item_name);
                $('#edit_status').prop('checked', item_active);                            
                    ...
                    $.ajax({
                        ...
                        error: function (xhr) {
                            ...
                            Swal.fire({
                                icon: 'error',
                                title: 'Error!',
                                text: 'Failed to create contract type.'
                            });
                        }
                    });                                    
                }
        ...
        function validateEditForm() {
            ...
            // Reset error messages
            $('#editContractTypeForm .error-message').addClass('hidden');
            $('#editContractTypeForm .form-select, #editContractTypeForm .form-input').removeClass('border-red-500');            
        ...
        function resetCreateForm() {
            $('#createContractTypeForm')[0].reset();
            $('#createContractTypeForm .error-message').addClass('hidden');
            $('#createContractTypeForm .form-select, #createContractTypeForm .form-input').removeClass('border-red-500');
        }
    ...
    <script>
        document.addEventListener('DOMContentLoaded', function() {
            ...
            document.querySelector('.reset-btn').addEventListener('click', function (e) {
                e.preventDefault();
                window.location = '{{ route('role.contract-types.index', ['role' => Str::slug(Auth::user()->getRoleNames()->first())]) }}';
            });
        });
    </script>
@endsection