...
@section('main-content')
        ...
        <div class="states-table-container">
            <div class="states-table-header bg-blue-600 px-6 py-4 flex justify-between items-center">
                <h2 class="states-table-title text-white text-xl font-semibold" style="color: white">
                    <i class="fas fa-list mr-2"></i>Lead Followup List
                </h2>
                @can('create lead followup')
                <button class="btn btn-primary create-btn bg-blue-500 hover:bg-blue-600 text-white px-4 py-2 rounded-md transition duration-200">
                    <i class="fas fa-plus mr-2"></i>Add Lead Followup
                </button>
                @endcan
            </div>
                        ...
                        <div class="filter-content">
                            <div class="filter-row">
                                ...
                                <div class="filter-group">
                                    ...
                                    <select name="user_id" id="filter_user" class="form-select w-full px-3 py-2 border border-gray-300 rounded-md">
                    ...
                    <table class="table table-hover min-w-full divide-y divide-gray-200">
                        <thead class="bg-gray-50">
                            <tr>
                                ...
                                @canany(['edit lead followup', 'delete lead followup']) 
                        ...
                        <tbody class="bg-white divide-y divide-gray-200">
                            @forelse ($datas as $key => $value)                       
                                <tr id="lead-followup-{{ $value->id }}">
                                    ...
                                    <td class="px-6 py-4 whitespace-nowrap">{{ \Carbon\Carbon::parse($value->followup_date)->format('Y-m-d') }}</td>                                                                     
                                    ...
                                    <td class="px-6 py-4 whitespace-nowrap">
                                        <div class="btn-group btn-group-sm flex space-x-1">
                                            ...
                                            <button class="btn btn-outline-primary edit-btn border border-blue-500 text-blue-500 hover:bg-blue-500 hover:text-white px-3 py-1 rounded-md transition duration-200" 
                                                ...
                                                data-action="{{ route('role.lead-followup.update', ['role' => Str::slug(Auth::user()->getRoleNames()->first()), 'lead_followup' => $value->id]) }}" title="Edit Item">
                                                <i class="fas fa-edit"></i>
                                            </button>
                                            @endcan
                                            @can('delete lead followup')
    ...
    @include('lead-followup.create-modal')
    @include('lead-followup.edit-modal')
    @include('lead-followup.delete-modal')
...
@section('js')
    ...
    <script>
        ...
        function validateCreateForm() {
            ...
            if (!$('#user_id').val().trim()) {
                $('#user_id').parent().find('.error-message').removeClass('hidden');
                $('#user_id').addClass('border-red-500');
                isValid = false;
            }
        ...
        function validateEditForm() {
            ...
            let editUserID = $('#edit_user_id').val();

            if (!editUserID || editUserID === '' || editUserID === 'undefined' || isNaN(editUserID)) {
                $('#edit_user_id').parent().find('.error-message').removeClass('hidden');
                $('#edit_user_id').addClass('border-red-500');
                isValid = false;
            }
    ...
    <script>
        document.addEventListener('DOMContentLoaded', function() {
            ...
            document.querySelector('.reset-btn').addEventListener('click', function (e) {
                e.preventDefault();
                window.location = "{{ route('role.lead-followup.index', ['role' => Str::slug(Auth::user()->getRoleNames()->first())]) }}";
            });
        });
    </script>
@endsection