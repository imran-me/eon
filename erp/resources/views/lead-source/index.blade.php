...
@section('main-content')
        ...
        <div class="states-table-container">
            <div class="states-table-header bg-blue-600 px-6 py-4 flex justify-between items-center">
                <h2 class="states-table-title text-white text-xl font-semibold" style="color: white">
                    <i class="fas fa-list mr-2"></i>Lead Source List
                </h2>
                ...
                <button class="btn btn-primary create-btn bg-blue-500 hover:bg-blue-600 text-white px-4 py-2 rounded-md transition duration-200">
                    <i class="fas fa-plus mr-2"></i>Add New Lead Source
                </button>
                @endcan
            </div>
                        ...
                        <tbody class="bg-white divide-y divide-gray-200">
                            @forelse ($datas as $key => $value)                       
                                <tr>
                                    ...
                                    @canany(['edit lead source', 'delete lead source'])                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                               
                                    <td class="px-6 py-4 whitespace-nowrap">
                                        <div class="btn-group btn-group-sm flex space-x-1">
                                            ...
                                            <button class="btn btn-outline-primary edit-btn border border-blue-500 text-blue-500 hover:bg-blue-500 hover:text-white px-3 py-1 rounded-md transition duration-200" 
                                                ...
                                                data-action="{{ route('role.lead-source.update', ['role' => Str::slug(Auth::user()->getRoleNames()->first()), 'lead_source' => $value->id]) }}" title="Edit Item">
    ...
    @include('lead-source.create-modal')
    @include('lead-source.edit-modal')
    @include('lead-source.delete-modal')
...
@section('js')
    ...
    <script>
                    ...
                    $.ajax({
                        ...
                        success: function (response) {
                            if (response.success) {
                                ...
                                $('#editLeadSourceModal').addClass('hidden');
                                setTimeout(() => window.location.reload(), 800);
                            } else {
                                Swal.fire({
                                    icon: "error",
        ...
        function validateEditForm() {
            ...
            // Reset error messages
            $('#editLeadSourceForm .error-message').addClass('hidden');
            $('#editLeadSourceForm .form-select, #editLeadSourceForm .form-input').removeClass('border-red-500');            
    ...
    <script>
        document.addEventListener('DOMContentLoaded', function() {
            ...
            document.querySelector('.reset-btn').addEventListener('click', function (e) {
                e.preventDefault();
                window.location = '{{ route('role.lead-source.index', ['role' => Str::slug(Auth::user()->getRoleNames()->first())]) }}';
            });
        });
    </script>
@endsection