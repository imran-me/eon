...
@section('main-content')
        ...
        <div class="states-table-container">
            <div class="states-table-header bg-blue-600 px-6 py-4 flex justify-between items-center">
                <h2 class="states-table-title text-white text-xl font-semibold" style="color: white">
                    <i class="fas fa-list mr-2"></i>SMS Templates List
                </h2>
                ...
                <button class="btn btn-primary create-new-btn bg-blue-500 hover:bg-blue-600 text-white px-4 py-2 rounded-md transition duration-200">
                    <i class="fas fa-plus mr-2"></i>Add New SMS Template
                </button>
                @endcan
            </div>
                        ...
                        <tbody class="bg-white divide-y divide-gray-200">
                            @forelse ($datas as $key => $value) 
                                <tr>
                                    ...
                                    <td class="px-6 py-4 whitespace-nowrap">
                                        {{ is_array($value->variables) 
                                            ? implode(', ', $value->variables) 
                                            : implode(', ', json_decode($value->variables, true) ?? []) }}
                                    </td>                                    
                                    <td class="px-6 py-4 whitespace-nowrap">
                                        @if ($value->status)
                                    ...
                                    <td class="px-6 py-4 whitespace-nowrap">
                                        <div class="btn-group btn-group-sm flex space-x-1">
                                            ...
                                            <button class="btn btn-outline-primary edit-item-btn border border-blue-500 text-blue-500 hover:bg-blue-500 hover:text-white px-3 py-1 rounded-md transition duration-200" 
                                                ...
                                                data-variables="{{ implode(', ', json_decode($value->variables ?? '[]', true)) }}"
    ...
    @include('sms_templates.create-modal')
    @include('sms_templates.edit-modal')
    @include('sms_templates.delete-modal')
...
@section('js')
    ...
    <script>
        ...
        function validateCreateForm() {
            ...
            if (!$('#create_template').val().trim()) {
                $('#create_template').next('.error-message').removeClass('hidden');
                $('#create_template').addClass('border-red-500');
                isValid = false;
            }      
            if (!$('#create_variables').val().trim()) {
                $('#create_variables').next('.error-message').removeClass('hidden');
                $('#create_variables').addClass('border-red-500');
                isValid = false;
            }                                                                       
        ...
        function validateEditForm() {
            ...
            if (!$('#edit_template').val().trim()) {
                $('#edit_template').next('.error-message').removeClass('hidden');
                $('#edit_template').addClass('border-red-500');
                isValid = false;
            }      
            if (!$('#edit_variables').val().trim()) {
                $('#edit_variables').next('.error-message').removeClass('hidden');
                $('#edit_variables').addClass('border-red-500');
                isValid = false;
            }                                            
    ...
    <script>
        document.addEventListener('DOMContentLoaded', function() {
            ...
            document.querySelector('.reset-btn').addEventListener('click', function (e) {
                e.preventDefault();
                window.location = '{{ route('role.sms_templates.index', ['role' => Str::slug(Auth::user()->getRoleNames()->first())]) }}';
            });
        });
    </script>
@endsection