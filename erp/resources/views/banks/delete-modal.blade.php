<div id="deleteModal" class="fixed inset-0 z-50 flex items-center justify-center p-4 hidden">
    ...
    <div class="relative w-full max-w-md bg-white rounded-2xl overflow-hidden shadow-2xl z-10">
        <div class="flex items-center gap-3 bg-gradient-to-r from-red-600 to-rose-500 px-5 py-4">
            ...
            <button type="button" class="modal-close-delete h-8 w-8 rounded-full bg-white/20 border-0 text-white cursor-pointer flex items-center justify-center">
        ...
        <div class="flex justify-end gap-2 bg-slate-50 px-5 py-3">
            <button type="button" class="modal-close-delete rounded-xl border border-slate-200 bg-white text-slate-700 px-4 py-2 text-sm font-semibold">Cancel</button>
            <button id="confirmDeleteBtn" data-action="{{ route('role.banks.destroy', ['role' => Str::slug(Auth::user()->getRoleNames()->first()), 'bank' => 1]) }}"
