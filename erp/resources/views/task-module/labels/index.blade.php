...
@section('main-content')
        ...
        <div class="states-table-container">
            <div class="states-table-header bg-blue-600 px-6 py-4 flex justify-between items-center">
                <h2 class="states-table-title text-white text-xl font-semibold" style="color: white">
                    <i class="fas fa-list mr-2"></i>Labels List
                </h2>
                @can('create label')
                <button class="btn btn-primary create-btn bg-blue-500 hover:bg-blue-600 text-white px-4 py-2 rounded-md transition duration-200">
                    <i class="fas fa-plus mr-2"></i>Add New Label
                </button>
                @endcan
            </div>
                    ...
                    <table class="table table-hover min-w-full divide-y divide-gray-200">
                        <thead class="bg-gray-50">
                            <tr>
                                ...
                                @can('any', ['edit label', 'delete label'])
                                ...
                                <tr>
                                    ...
                                    <td class="px-6 py-4 whitespace-nowrap">
                                        <span class="px-3 py-1 rounded-lg text-xs font-medium {{ $value->color_details['text'] }}" style="background: {{ $value->color_details['gradient'] }}">
                                            {{ Str::startsWith($value->color ?? '', '#') ? strtoupper($value->color) : ucfirst($value->color ?? 'blue') }}
                                        </span>
                                    </td>
                                    <td class="px-6 py-4 whitespace-nowrap">
                                        <small class="text-gray-500">{{ date('M d, Y', strtotime($value->created_at)) }}</small>
                                    </td>
                                    @canany(['edit label', 'delete label'])                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                            
                                    <td class="px-6 py-4 whitespace-nowrap">
    ...
    @include('task-module.labels.create-modal')
    @include('task-module.labels.edit-modal')
    @include('task-module.labels.delete-modal')
...
@section('js')
    ...
    <script>
        $(document).ready(function() {
            ...
            const presetColors = ['gray', 'blue', 'purple', 'green', 'yellow', 'red', 'indigo', 'pink', 'orange', 'teal'];

            function clearCreateCustomColorState() {
                $('#create_custom_color').closest('div').removeClass('ring-4 ring-blue-500');
            }

            function clearEditCustomColorState() {
                $('#edit_custom_color').closest('div').removeClass('ring-4 ring-blue-500');
            }
            ...
            $('.create-btn').click(function() {
                ...
                $('#createForm')[0].reset();
                $('#create_color').val('blue');
                $('#create_custom_color').val('#3b82f6');
                $('#create_custom_color_value').text('#3b82f6');
                $('.error-message').addClass('hidden');
                $('.color-option').removeClass(color);
                clearCreateCustomColorState();
                $('input[name="color_preset"][value="blue"]')
                    .prop('checked', true)
                    .closest('label')
                    .find('.color-option')
            ...
            $('.color-option').on('click', function() {
                ...
                $('#create_color').val(selectedColor);
                clearCreateCustomColorState();
                $(this).addClass('ring-4 ring-' + selectedColor + '-500');
            });

            $('#create_custom_color').on('input', function() {
                const selectedColor = $(this).val();
                $('#create_color').val(selectedColor);
                $('#create_custom_color_value').text(selectedColor);
                $('input[name="color_preset"]').prop('checked', false);
                $('.color-option').removeClass(color);
                $(this).closest('div').addClass('ring-4 ring-blue-500');
            });
            ...
            $('.color-option-edit').on('click', function() {
                ...
                $radio.prop('checked', true);
                $('#edit_color').val(selectedColor);
                clearEditCustomColorState();
                $(this).addClass('ring-4 ring-' + selectedColor + '-500');
            });

            $('#edit_custom_color').on('input', function() {
                const selectedColor = $(this).val();
                $('#edit_color').val(selectedColor);
                $('#edit_custom_color_value').text(selectedColor);
                $('input[name="color_preset"]').prop('checked', false);
                $('.color-option-edit').removeClass(color);
                $(this).closest('div').addClass('ring-4 ring-blue-500');
            });
            ...
            $('.edit-btn').click(function() {
                ...
                if (presetColors.includes(selectedColor)) {
                    $('input[name="color_preset"][value="' + selectedColor + '"]')
                        .prop('checked', true)
                        .siblings('.color-option-edit')
                        .addClass('ring-4 ring-' + selectedColor + '-500');
                    $('#edit_custom_color').val('#3b82f6');
                    $('#edit_custom_color_value').text('#3b82f6');
                } else {
                    $('input[name="color_preset"]').prop('checked', false);
                    $('#edit_custom_color').val(selectedColor);
                    $('#edit_custom_color_value').text(selectedColor);
                    $('#edit_custom_color').closest('div').addClass('ring-4 ring-blue-500');
                }
    ...
    <script>
        document.addEventListener('DOMContentLoaded', function() {
            ...
            document.querySelector('.reset-btn').addEventListener('click', function (e) {
                e.preventDefault();
                window.location = "{{ route('role.labels.index', ['role' => Str::slug(Auth::user()->getRoleNames()->first())]) }}";
            });
        });
    </script>
@endsection
