...
@section('main-content')
        ...
        <div class="states-table-container">
            <div class="states-table-header bg-blue-600 px-6 py-4 flex justify-between items-center">
                <h2 class="states-table-title text-white text-xl font-semibold" style="color: white">
                    <i class="fas fa-list mr-2"></i>Project Category List
                </h2>
                <button class="btn btn-primary create-btn bg-blue-500 hover:bg-blue-600 text-white px-4 py-2 rounded-md transition duration-200">
                    <i class="fas fa-plus mr-2"></i>Add Project Category
                </button>
            </div>
                        ...
                        <div class="filter-content">
                            <div class="filter-row">
                                <div class="filter-group">
                                    ...
                                    <select name="company_id" id="filter_company_id" class="select2">
                                    ...
                                    <td class="px-6 py-4 whitespace-nowrap">
                                        <div class="btn-group btn-group-sm flex space-x-1">
                                            <button class="btn btn-outline-primary edit-btn border border-blue-500 text-blue-500 hover:bg-blue-500 hover:text-white px-3 py-1 rounded-md transition duration-200"
                                                ...
                                                data-action="{{ route('role.project-categories.update', ['role' => Str::slug(Auth::user()->getRoleNames()->first()), 'project_category' => $value->id]) }}" title="Edit Item">
    ...
    @include('project-categories.create-modal')
    @include('project-categories.edit-modal')
    @include('project-categories.delete-modal')
    ...
    <script>
        $(document).ready(function() {
            ...
            $('.edit-btn').click(function() {
                ...
                const item_company_id = $(this).data('company_id');
                const item_action = $(this).data('action');

                $('#editItemId').val(item_id);
                $('#edit_name').val(item_name);
                $('#edit_status').prop('checked', item_active);
                $('#edit_company_id').val(item_company_id).trigger('change');
                $('#editSubmit').data('action', item_action);
                    ...
                    $.ajax({
                        ...
                        error: function (xhr) {
                            ...
                            Swal.fire({
                                icon: 'error',
                                title: 'Error!',
                                text: 'Failed to create lead-source.'
                            });
                        }
                    });                                    
                }
    ...
    <script>
        document.addEventListener('DOMContentLoaded', function() {
            ...
            document.querySelector('.reset-btn').addEventListener('click', function (e) {
                e.preventDefault();
                window.location = '{{ route('role.project-categories.index', ['role' => Str::slug(Auth::user()->getRoleNames()->first())]) }}';
            });
        });
    </script>
@endsection